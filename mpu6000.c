...
static int32_t gyro_pitch_cal, gyro_roll_cal, gyro_yaw_cal, cal_int;
...
void readMPU6000(void) {
	...
	acc_pitch = (int16_t) (H << 8) | L;
	
	// accel y
	H = SPITransfer(MPU6000_SPI, 0x00);
	L = SPITransfer(MPU6000_SPI, 0x00);
	acc_roll = (int16_t) (H << 8) | L;
	
	// accel z
	H = SPITransfer(MPU6000_SPI, 0x00);
	L = SPITransfer(MPU6000_SPI, 0x00);
	acc_yaw = (int16_t) (H << 8) | L;
	...
	H = SPITransfer(MPU6000_SPI, 0x00);
	L = SPITransfer(MPU6000_SPI, 0x00);
	gyro_roll = (int16_t) (H << 8) | L;
	
	// gyro y
	H = SPITransfer(MPU6000_SPI, 0x00);
	L = SPITransfer(MPU6000_SPI, 0x00);
	gyro_pitch = (int16_t) (H << 8) | L;
	
	// gyro z
	H = SPITransfer(MPU6000_SPI, 0x00);
	L = SPITransfer(MPU6000_SPI, 0x00);
	gyro_yaw = (int16_t) (H << 8) | L;
...
void calibrateMPU6000(void) {
	...
	printMsg("%d %d %d %d\n", gyro_pitch_cal, gyro_roll_cal, gyro_yaw_cal, cal_int);
